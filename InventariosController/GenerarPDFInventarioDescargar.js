...
async function GenerarPDFInventarioDescargar(req, res) {
    try {
        ...
        // Función para formatear moneda (con signo $)
        const formatearMoneda = (valor) => {
            // Se usa minimumFractionDigits: 0 para que no muestre decimales si son .00
            // y maximumFractionDigits: 2 para que muestre hasta 2 decimales si son necesarios.
        ...
        if (detallesProductos.length > 0) {
            ...
            const productTableHeaders = [
                ...
                { text: 'PRECIO', x: 425, width: 60, align: 'right' }, // Alineado a la derecha
                { text: 'SUBTOTAL', x: 490, width: 60, align: 'right' } // Alineado a la derecha
            ];

            doc.fontSize(9).font('Helvetica-Bold');
            productTableHeaders.forEach(header => {
                doc.text(header.text, header.x, yPosition, { width: header.width, align: header.align || 'left' });
            });
            ...
            detallesProductos.forEach((detalle, index) => {
                ...
                const precioTexto = formatearMoneda(detalle.precio_unitario);
                const subtotalTexto = formatearMoneda(detalle.subtotal);

                const rowData = [
                    { text: detalle.referencia.substring(0, 8), x: 50, width: 50 },
                    { text: detalle.nombre.substring(0, 18), x: 105, width: 120 },
                    { text: (detalle.nombre_categoria || 'N/A').substring(0, 12), x: 230, width: 80 },
                    { text: (detalle.marca || 'N/A').substring(0, 10), x: 315, width: 60 },
                    { text: detalle.cantidad.toString(), x: 380, width: 40, align: 'center' }, // Centrado
                    { text: precioTexto, x: 425, width: 60, align: 'right' }, // Alineado a la derecha
                    { text: subtotalTexto, x: 490, width: 60, align: 'right' } // Alineado a la derecha
                ];

                rowData.forEach(cell => {
                    doc.text(cell.text, cell.x, yPosition, { width: cell.width, align: cell.align || 'left' });
                });
        ...
        if (detallesCalcomanias.length > 0) {
            ...
            const calcomaniaTableHeaders = [
                ...
                { text: 'NOMBRE', x: 85, width: 90 },
                { text: 'STOCK PEQ.', x: 180, width: 60, align: 'center' },
                { text: 'STOCK MED.', x: 245, width: 60, align: 'center' },
                { text: 'STOCK GDE.', x: 310, width: 60, align: 'center' },
                { text: 'TOTAL UNID.', x: 375, width: 60, align: 'center' },
                { text: 'PRECIO', x: 440, width: 55, align: 'right' }, // Más ancho y alineado a la derecha
                { text: 'SUBTOTAL', x: 500, width: 55, align: 'right' } // Más ancho y alineado a la derecha
            ];
            ...
            detallesCalcomanias.forEach((detalle, index) => {
                ...
                const rowData = [
                    { text: detalle.id_calcomania.toString(), x: 50, width: 30 },
                    { text: detalle.nombre.substring(0, 15), x: 85, width: 90 },
                    { text: detalle.stock_pequeno.toString(), x: 180, width: 60, align: 'center' },
                    { text: detalle.stock_mediano.toString(), x: 245, width: 60, align: 'center' },
                    { text: detalle.stock_grande.toString(), x: 310, width: 60, align: 'center' },
                    { text: detalle.stock_general.toString(), x: 375, width: 60, align: 'center' },
                    { text: precioTexto, x: 440, width: 55, align: 'right' },
                    { text: subtotalTexto, x: 500, width: 55, align: 'right' }
                ];
